...
struct WaterVertexOutput // fragment struct
{
	...
	float4	additionalData			: TEXCOORD5;	// x = distance to surface, y = local position in camera space, z = normalized wave height, w = horizontal movement
...
half4 WaterFragment(WaterVertexOutput IN) : SV_Target
{
	...
	half foamBlendMask = max(waveFoam, edgeFoam);//max(, waterFX.r * 2);
