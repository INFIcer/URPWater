...
namespace WaterSystem
{
	...
	public class Water : MonoBehaviour
	{
		...
		// 			transform.localScale); // transform.localToWorldMatrix;
		//
		...
		private void SetWaves()
		{
			...
			m_Material.SetTexture(FoamMap, resources.defaultFoamMap);
			m_Material.SetTexture(SurfaceMap, resources.defaultSurfaceMap);
			...
			m_Material.SetFloat(WaveHeight, _waveHeight);
			m_Material.SetFloat(MaxWaveHeight, _maxWaveHeight);
			m_Material.SetFloat(MaxDepth, surfaceData._waterMaxVisibility);

			switch (settingsData.refType)
			{
				case ReflectionType.Cubemap:
					m_Material.EnableKeyword("_REFLECTION_CUBEMAP");
					m_Material.DisableKeyword("_REFLECTION_PROBES");
					m_Material.DisableKeyword("_REFLECTION_PLANARREFLECTION");
					m_Material.SetTexture(CubemapTexture, settingsData.cubemapRefType);
					break;
				case ReflectionType.ReflectionProbe:
					m_Material.DisableKeyword("_REFLECTION_CUBEMAP");
					m_Material.EnableKeyword("_REFLECTION_PROBES");
					m_Material.DisableKeyword("_REFLECTION_PLANARREFLECTION");
					break;
				case ReflectionType.PlanarReflection:
					m_Material.DisableKeyword("_REFLECTION_CUBEMAP");
					m_Material.DisableKeyword("_REFLECTION_PROBES");
					m_Material.EnableKeyword("_REFLECTION_PLANARREFLECTION");
					break;
				default:
					throw new ArgumentOutOfRangeException();
			}

			m_Material.SetInt(WaveCount, _waves.Length);

			//GPU side
			if (_useComputeBuffer)
			{
				m_Material.EnableKeyword("USE_STRUCTURED_BUFFER");
				waveBuffer?.Dispose();
				waveBuffer = new ComputeBuffer(10, (sizeof(float) * 6));
				waveBuffer.SetData(_waves);
				m_Material.SetBuffer(WaveDataBuffer, waveBuffer);
			}
			else
			{
				m_Material.DisableKeyword("USE_STRUCTURED_BUFFER");
				m_Material.SetVectorArray(WaveData, GetWaveData());
			}
		...
		private void GenerateColorRamp()
		{
			...
			for (var i = 0; i < 128; i++)
			{
				switch (surfaceData._foamSettings.foamType)
				{
					...
					case 1: // simple
						cols[i + 256] =
							defaultFoamRamp.GetPixelBilinear(surfaceData._foamSettings.basicFoam.Evaluate(i / 128f),
								0.5f);
						break;
					case 2: // custom
